...
void
GAM_setDetailLevel(int n)
{
     detailLevel=n;
}

int
GAM_getDetailLevel(void)
{
     return(detailLevel);
}
...
GAM_updateStatusBar(void)
{
     ...
     if (viewSize <= xdim) {
          rotatesprite(320<<15,(200-(tilesizy[statusBarPic]>>1))<<16,
                       65536L,0,statusBarPic,0,0,2+8+64+128,
...
GAM_setupVideo(int vsize)
{
     ...
     if (viewSize > xdim) {
          ...
          x1=((xdim-viewSize)>>1);
          x2=x1+viewSize-1;
          scaledTileY=scale(ydim,100L,200L);
          scaledTileY=scale(scaledTileY,tilesizy[statusBarPic],100L);
          y1=(((ydim-scaledTileY)-scale(viewSize,ydim-scaledTileY,xdim))>>1);
          y2=y1+scale(viewSize,ydim-scaledTileY,xdim)-1;
          GAM_updateStatusBar();
     }
     setview(x1,y1,x2,y2);
...
void
GAM_scanDetailSprites(void)
{
     int  detail,i;
     spritetype *spr;
     
     detail=GAM_getDetailLevel();
...
void
GAM_underWater(short s,short flag)
{
     if (flag) {
          underWater[s>>3]|=(1<<(s&7));
     }
     else {
          underWater[s>>3]&=~(1<<(s&7));
     }
...
int
GAM_isUnderWater(short s)
{
     if ((underWater[s>>3]&(1<<(s&7))) != 0) {
          return(1);
     }
     return(0);
}
...
int
GAM_isOnWater(short s)
{
     if (EFF_isAboveWaterSector(spritePtr[s]->sectnum)) {
          if (GAM_isOnGround(s)) {
               return(1);
          }
     }
...
int
GAM_doWaterPhysics(short s)
{
     if (GAM_isUnderWater(s) || GAM_isOnWater(s)) {
          return(1);
     }
     return(0);
}
...
GAM_doFrame(void)
{
     ...
     PLR_checkStatusBar();
     ENG_drawScreen(viewSprite,(totalclock-gotLastPacketClock)*(65536/TICWAITPERFRAME));
...
void
GAM_doPhysics(short s,spritetype *spr,long *x,long *y,long *z,short *sect,
              short zAdj,short zHeight,short clipheight)
{
     long goalz,newzvel;
     
//** find the Z point of view we want to be at
     newzvel=spr->zvel;
     if (*z > (globloz-clipheight) || *z < (globhiz+clipheight)) {
          getzsofslope(spr->sectnum,spr->x,spr->y,&globhiz,&globloz);
     }
     goalz=globloz-zAdj;
#if 0
     if (goalz < globhiz+clipheight) {
          goalz=(globhiz+clipheight);
     }
#endif
     if (GAM_doWaterPhysics(s) && GAM_isOnGround(s)) {
          goalz=globloz-(clipheight<<6);
     }
//** apply physics to current Z until goalz reached
     *z+=newzvel;
     if (*z > globloz-clipheight) {     // too close to floor
          if (GAM_isOnWater(s)) {
               EFF_warpBelowWater(s,spr,sect,x,y,z);
          }
          else {
               *z=globloz-clipheight;
               newzvel=0L;
          }
     }
     if (*z < globhiz+clipheight) {     // too close to ceiling
          if (GAM_isUnderWater(s)) {
               EFF_warpAboveWater(s,spr,sect,x,y,z);
          }
          else {
               *z=globhiz+clipheight;
               newzvel=0L;
          }
     }
     if (GAM_doWaterPhysics(s)) {
          *z+=sintable[(fakeClock<<3)&2047]>>9;
          if (newzvel > 0L) {
               newzvel=kmax(newzvel-(TICWAITPERFRAME<<4),0L);
          }
          else if (newzvel < 0L) {
               newzvel=kmin(newzvel+(TICWAITPERFRAME<<4),0L);
          }
     }
     if (!GAM_isUnderWater(s)) {
          if (newzvel <= 0L) {     // don't hop if going down a slope
               if (*z < goalz && goalz-*z < clipheight) {
                    *z=goalz;
               }
          }
          if (*z < globloz-zHeight) {        // off the ground
               GAM_onGround(s,0);
               ...
               else if (!GAM_isOnWater(s)) {
                    newzvel=(((goalz-*z)*TICWAITPERFRAME)>>4);
               }
          }
          if (newzvel > 32767L) {
               newzvel=32767L;
