...
int
PLR_isPlayer(short s)
{
     if (spritePtr[s]->statnum == STAT_PLAYER) {
          return(1);
     }
     return(0);
}
...
PLR_updateStatusHealth(void)
{
     ...
     for (i=0 ; i < strlen(locbuf) ; i++) {
          ...
          rotatesprite(x<<16,y<<16,65536L,0,pic,0,0,8+128,0L,0L,xdim-1L,ydim-1L);
          x+=tilesizx[pic];
     }
}
          ...
          if ((sectPtr->floorstat&0x02) != 0) {   // on a slope
               ...
               if (horizCheckSect != -1) {
                    horizCheckZ=getflorzofslope(horizCheckSect,
                                                horizCheckX,
                                                horizCheckY);
                    horizSlopeGoal[s]=kmin((spr->z-horizCheckZ)>>7,200);
...
//** do game physics
     GAM_doPhysics(s,&z,&spr->zvel,zAdj,playerHeight,clipheight);
//**
...
PLR_debug(void)
{
     ...
     for (p=connecthead ; p >= 0 ; p=connectpoint2[p]) {
          s=PLR_getPlayerSprite(p);
          spr=spritePtr[s];
          debugOut(windowx1,windowy1,"P=%d X=%06d Y=%06d Z=%06d A=%04d H=%d "
                   "AM=%d G=%d HOR=%d",p,spr->x,spr->y,spr->z,spr->ang,
                   ACT_getHealth(s),WEP_getAmmo(s,WEP_getSpriteAmmoType(s)),
                   GAM_isOnGround(s),spriteHoriz[s]);
          debugOut(windowx1,windowy1,"CDIST=%d CHEIGHT=%d PHEIGHT=%d",
                   ACT_getActorClipDist(spr),ACT_getActorClipHeight(spr),
                   ACT_getActorViewHeight(spr));
     }
